...
class Controller {
  async sendSms(req, res) {
    logger.info(`received POST request from ${req.originalUrl}`);
    const { toNumber } = req.body;
    const reqId = req.headers[constants.requestIdHdr];
    return manager.sendSms(toNumber, reqId)
      .then(() => {
        logger.info({ request_id: reqId }, `Sent sms successfully to number: ${toNumber}`);
        const response = new Response(
          toNumber,
          constants.sentSuccessfullyCode,
          constants.sentSuccessfullyMsg,
        );
